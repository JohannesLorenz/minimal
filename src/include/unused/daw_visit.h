...
// TODO: own header
class osc_con : non_copyable_t
{
private:
	static pid_t make_fork(const char *start_cmd);
	osc_con(const osc_con& other) = delete;
public:
	const pid_t pid;
	const int fd;
	...
	~osc_con();
	osc_con(const instrument_t& ins);
	osc_con(osc_con&) = delete;
	osc_con(osc_con&&) = default;
...
//	cmd_vectors visit(const track_t& t);
	...
	// osc port (via instrument), commands, times
